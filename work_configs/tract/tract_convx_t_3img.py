...
model = dict(
    ...
    decode_head=dict(
        ...
        norm_cfg=norm_cfg,
        align_corners=False,
        loss_decode=dict(type='DiceLoss', multi_label=True, loss_weight=1.0),
    ),
    auxiliary_head=dict(
        type='FCNHead',
        in_channels=384,
...
albu_train_transforms = [
    ...
    dict(type='Affine', scale=0.1, translate_percent=0.1, rotate=5, shear=5),
    dict(type='Cutout', p=0.5),
]
train_pipeline = [
    dict(type='Load3ImageFromFile'),
...
work_dir = f'./work_dirs/tract/convx_t_16x2_20k_aug2_3img_f0'
