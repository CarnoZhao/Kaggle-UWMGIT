...
model = dict(
    type='SMPUnet',
    backbone=dict(type='timm-efficientnet-b7', pretrained='noisy-student'),
    decode_head=dict(
        num_classes=3,
        align_corners=False,
        loss_decode=[
...
runner = dict(type='IterBasedRunner', max_iters=35400)
checkpoint_config = dict(by_epoch=False, interval=590, save_optimizer=False)
evaluation = dict(
    by_epoch=False, interval=1180, metric=['imDice', 'mDice'], pre_eval=True)
fp16 = dict()
# work_dir = './work_dirs/new/baseline_640_b7_bs14_mask_fold1'
work_dir = './work_dirs/tract/seg_3'
gpu_ids = range(0, 2)
