...
model = dict(
    ...
    decode_head=dict(
        ...
        loss_decode=[
            ...
            dict(type='SMPDiceLoss', mode='multilabel', loss_weight=3.0)
        ]),
    train_cfg=dict(),
...
data = dict(
    ...
    train=dict(
        ...
        img_suffix='.png',
        seg_map_suffix='.png',
        split='splits_noanno/fold_all.txt',
        classes=['large_bowel', 'small_bowel', 'stomach'],
        palette=[[0, 0, 0], [128, 128, 128], [255, 255, 255]],
        pipeline=[
            dict(
...
runner = dict(type='IterBasedRunner', max_iters=16500)
checkpoint_config = dict(by_epoch=False, interval=330, save_optimizer=False)
evaluation = dict(
    by_epoch=False, interval=660, metric=['imDice', 'mDice'], pre_eval=True)
fp16 = dict()
# work_dir = './work_dirs/new/baseline_640_b4_bs25_mask_fold'

work_dir = './work_dirs/tract/seg_2'
gpu_ids = range(0, 2)
