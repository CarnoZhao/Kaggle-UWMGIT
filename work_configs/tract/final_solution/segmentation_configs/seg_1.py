...
model = dict(
    type='SMPUnet',
    backbone=dict(type='timm-efficientnet-b6', pretrained='noisy-student'),
    decode_head=dict(
        num_classes=3,
        align_corners=False,
        loss_decode=[
...
checkpoint_config = dict(by_epoch=False, interval=520, save_optimizer=False)
evaluation = dict(
    by_epoch=False, interval=1040, metric=['imDice', 'mDice'], pre_eval=True)
fp16 = dict()
# work_dir = './work_dirs/new/baseline_640_b6_bs16_mask_fold'
work_dir = './work_dirs/tract/seg_1'
gpu_ids = range(0, 2)
