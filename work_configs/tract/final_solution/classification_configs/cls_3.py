...
model = dict(
    type='SMPUnet',
    backbone=dict(type='timm-efficientnet-b4', pretrained='noisy-student'),
    decode_head=dict(
        num_classes=3,
        align_corners=False,
        loss_decode=[
            dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=3.0),
...
data = dict(
    ...
    train=dict(
        type='CustomDataset',
        multi_label=True,
        data_root='./data/tract/mmseg_train/',
        img_dir='images',
        ann_dir='labels',
        img_suffix='.png',
        seg_map_suffix='.png',
        split='splits_case/fold_all.txt',
        classes=['large_bowel', 'small_bowel', 'stomach'],
        palette=[[0, 0, 0], [128, 128, 128], [255, 255, 255]],
        pipeline=[
            dict(
...
runner = dict(type='IterBasedRunner', max_iters=38200)
checkpoint_config = dict(by_epoch=False, interval=955, save_optimizer=False)
evaluation = dict(
    by_epoch=False, interval=955, metric=['imDice', 'mDice'], pre_eval=True)
fp16 = dict()
# work_dir = './work_dirs/new/baseline_640_b4_bs16_Albu_fold1'
work_dir = './work_dirs/tract/cls_3'
gpu_ids = range(0, 2)
